...
{
    using namespace juce;
    // (Наш компонент непрозрачен, поэтому мы должны полностью заполнить фон сплошным цветом)
...
void SimpleEQAudioProcessorEditor::resized()
{
    ...
    auto responseArea = bounds.removeFromTop(bounds.getHeight() * hRatio); //измените значение с 0,33 на 0,25, потому что мне нужен был текст с пиковой частотой, чтобы он не перекрывал большой палец ползунка
